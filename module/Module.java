...
public abstract class Module {
	...
	public void onEnableTranslation() {
		DecelerateAnimation smooth = new DecelerateAnimation(1000, mc.displayWidth / 2 + this.displayName.length() + 4,
				Direction.BACKWARDS);
		if (smooth.getDirection() == Direction.FORWARDS) {
			smooth.changeDirection();
		}
		translation = (float) smooth.getOutput();
	}

	public void onDisableTranslation() {
		DecelerateAnimation smooth = new DecelerateAnimation(1000, mc.displayWidth / 2 + 4, Direction.FORWARDS);
		if (smooth.getDirection() == Direction.BACKWARDS) {
			smooth.changeDirection();
		}
		translation = (float) smooth.getOutput();
	}
	...
	public void toggle() {
		...
		if (enabled) {
//			NotificationManager.getNotificationManager().createNotification("Info", "Enabled " + this.getName(), true,
//					1500, Type.INFO, Color.GREEN);
			onEnable();
		}
		if (!enabled) {
//			NotificationManager.getNotificationManager().createNotification("Info", "Disabled " + this.getName(), true,
//					1500, Type.INFO, Color.RED);
			onDisable();
		}
	}
