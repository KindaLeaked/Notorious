...
public class ChestStealer extends Module {
	...
	public void onEvent(Event e) {
		if (e instanceof EventUpdate) {
			speed = (int) Client.instance.settingsManager.getSettingByName("stealerSpeed").getValDouble();
			if (wasNotChest && mc.currentScreen instanceof GuiChest) {
				startDelay.reset();
			}
			wasNotChest = !(mc.currentScreen instanceof GuiChest);

			if (mc.currentScreen instanceof GuiChest && startDelay.getTime() > 250 && invCooldownElapsed((long) (1000
					/ Math.max(1, (speed + (int) (Math.random() * randomization - randomization / 2f)))))) {
				...
				for (Slot slot : container.inventorySlots) {
					...
					if (id <= 26 && item != null && shouldSteal(item)) {
						...
						for (Slot playerSlot : mc.thePlayer.inventoryContainer.inventorySlots) {
							...
							if (empty) {
								// mc.thePlayer.addChatMessage(new ChatComponentText(id + " " +
								// item.toString()));
	...
	public static float getSwordStrength(ItemStack stack) {
		return (!(stack.getItem() instanceof ItemSword) ? 0.0F
				...
				+ (!(stack.getItem() instanceof ItemSword) ? 0.0F
						: (float) EnchantmentHelper.getEnchantmentLevel(Enchantment.fireAspect.effectId, stack) * 1F);
	}
		...
		for (int i = 9; i < 45; i++) {
			...
			if (stack != null && compareStack != stack && stack.getItem() instanceof ItemSword) {
				...
				if (item.getClass() == compare.getClass()) {
					if (compare.attackDamage + getSwordStrength(compareStack) <= item.attackDamage
							+ getSwordStrength(stack))
						return false;
				}
			}
		}
	...
	public boolean isBestTool(ItemStack compareStack) {
		for (int i = 9; i < 45; i++) {
			...
			if (stack != null && compareStack != stack && stack.getItem() instanceof ItemTool) {
				...
				if (item.getClass() == compare.getClass()) {
					if (compare.getStrVsBlock(stack, preferredBlock(item.getClass())) <= item
							.getStrVsBlock(compareStack, preferredBlock(compare.getClass())))
						return false;
				}
			}
		}
