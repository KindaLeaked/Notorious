package Notorious.notification;
...
public class Notification {
	...
	public Notification(String title, String text, boolean showTimer, long timeOnScreen, Type type, Color color,
			int targetX, int targetY, int startingX, int startingY, int speed) {
		font = Client.instance.font20;
		this.title = title;
		this.text = text;
	...
	public void onRender(int Num) {
		...
		if ((font.getStringWidth(title) * 1.1) + 45 >= 170) {

			notWidth = (font.getStringWidth(title) * 1.1) + 50;
			startingX -= (font.getStringWidth(title) * 1.1) - 170 + 50;

		}

		if ((font.getStringWidth(text) * 0.9) + 43.33 >= notWidth) {

			notWidth = (font.getStringWidth(text) * 0.9) + 50;
			startingX -= (font.getStringWidth(text) * 0.9) - 170 + 50;
		...
		GlStateManager.translate(-4, -4, 0);
		font.drawString(title, (int) ((startingX + 45) / 1.1),
				(int) ((int) ((startingY + 9 - (font.getFontHeight() / 2))) / 1.1), color.color, false);
		...
		GlStateManager.translate(-4, -4, 0);
		font.drawString(text, (float) ((startingX + 43.33) / 0.9),
				(float) ((float) ((startingY + 11 + (font.getFontHeight()))) / 0.9), color.color, false);
		GlStateManager.popMatrix();

		mc.getTextureManager().bindTexture(
				new ResourceLocation("Notorious/notifications/" + type.filePrefix + color.fileSuffix + ".png"));
		int size = 30;
		...
		if (showTimer && timeOnScreen - System.currentTimeMillis() > 0) {
			...
			Gui.drawRect(startingX, startingY + 40,
					startingX + ((notWidth / originalTime) * ((timeOnScreen - System.currentTimeMillis()))),
					startingY + 45, color.color);
